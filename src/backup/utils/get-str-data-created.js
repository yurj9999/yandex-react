...
export const getStrDataCreated = createdAt => {
    const checkForZeroes = timeCount => timeCount < WITHOUT_ZERO_MAX_TIME ? `0${timeCount}` : timeCount;

    const getDaysInterval = (nowDate, agoDate) => {
        const days = Math.round((nowDate - agoDate) / MILLISECONDS_IN_DAY);
